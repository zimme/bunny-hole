...
async function checkWorkflows(
  ...
  for (const name of WORKFLOWS) {
    ...
    if (name === "check.yml") {
      ...
      const push = typeof trigger === "object" && trigger !== null
        ? (trigger as Record<string, unknown>).push
        : undefined;
      ...
      if (push !== null && typeof push === "object") {
        failures.push(
          `${TEMPLATE_ROOT}/${rel}: required check workflow must run on pushes to every consumer default branch`,
        );
      }
      ...
      if (!/terraform\s+-chdir=terraform\s+test\s+-no-color/.test(source)) {
        failures.push(
      ...
      if (!/gh\[[^\]]*r[^\]]*\]/.test(source)) {
        failures.push(
          `${TEMPLATE_ROOT}/${rel}: secret scan must detect GitHub refresh tokens`,
        );
      }
      if (
        !/grep_status\s*=\s*\$\?/.test(source) ||
        !/grep_status\s*!==?\s*1/.test(source)
      ) {
        failures.push(
          `${TEMPLATE_ROOT}/${rel}: secret scan must fail closed on git grep errors`,
        );
      }
