...
variable "dns_zone_domain" {
  ...
  validation {
    condition     = var.dns_zone_domain == null || (length(var.dns_zone_domain) <= 253 && can(regex("^[a-z0-9](?:[a-z0-9-]{0,61}[a-z0-9])?(?:\\.[a-z0-9](?:[a-z0-9-]{0,61}[a-z0-9])?)*$", var.dns_zone_domain)) && !strcontains(var.dns_zone_domain, "*"))
