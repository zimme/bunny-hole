...
Read `AGENTS.md`, `README.md`, tracked Terraform configuration and example files under
`terraform/` (including the provider lock file), and the
[sanitized handoff schema](references/handoff-schema.md) before acting. Do not read
Terraform state, plans, local `.tfvars` files, credentials, or other ignored artifacts.
This is a consumer infrastructure repository; do not modify Bunny Hole
application/protocol code or substitute an unpublished image.
