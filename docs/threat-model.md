...
credentials, relay availability, and released image integrity. Untrusted parties include
public HTTP clients, unauthenticated WebSocket peers, compromised networks, malicious
origins, and contributors modifying CI.
...
connectors trust the configured relay hostname and their configured origin.
...
- **Supply chain:** frozen lockfile, minimal runtime dependencies, pinned toolchain and
  actions, tag-only releases, SBOM and provenance attestations, CodeQL, dependency
  review, Scorecard, and reproducible local validation.
