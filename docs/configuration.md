...
A record has `id`, a 32-byte base64url Ed25519 `publicKey`, and one or more exact
...
strictly. The relay never receives a connector private key.
...
| ---------------------------------- | --------- | ------------------------------------------ |
| `BUNNY_HOLE_TUNNELS`               | yes       | Same public tunnel records as the relay    |
...
tunnel IDs, hostnames, records, or private keys. Remove the token after testing.

## Connector

Flags override environment, which overrides a JSON `--config` file except the private
key: it is intentionally not accepted as a flag. On Unix, config files must be mode
`0600` or stricter.
...
| config / `BUNNY_HOLE_TUNNEL_PRIVATE_KEY`                       | yes      | —                       |
...
The `generate` command emits a complete private connector config on stdout and the
matching public `BUNNY_HOLE_TUNNELS` array on stderr. Redirect both streams to separate
files under `umask 077`; only the public relay file belongs in Bunny.
